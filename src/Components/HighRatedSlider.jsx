...
function HighRatedSlider() {
    ...
    return (
        <div className="mt-15 relative px-4 sm:px-8 md:px-16 w-full select-none mx-auto">
            <div className="flex gap-270">
                <h1 className="text-5xl font-bold mb-6">Bests</h1>
                <Link
                    to="/ShowHighRated"
                    className="mt-5 text-[#8864FE] hover:text-[#4a3f75]"
                >
                    View top 100 Series
                </Link>
            </div>
                ...
                <CarouselContent className="-ml-4 py-2">
                    {topMovies.slice(0, 20).map((movie) => {
                        return (
                            <CarouselItem
                                key={movie.name}
                                className="basis-50 pl-4 object-cover"
                            >
                                <Card
                                    movie={movie}
                                    className="flex items-center justify-center py-2.5 px-5"
                                ></Card>
                            </CarouselItem>
                        );
                    })}
